            ...
            <div class="w-full md:w-3/12 md:mx-2">
                ...
                <div class="bg-white p-3 border-t-4 border-green-400">
                    <div class="image overflow-hidden">
                        ...
                        @else
                            <img class="h-auto w-full mx-auto" src="{{Storage::url($material->material_type->image->url)}}">
                        @endif
                            
                    </div>
                    <h1 class="text-gray-900 font-bold text-xl leading-8 my-1">{{$material->name}}</h1>
                    <h3 class="text-gray-600 font-lg text-semibold leading-6">{{$material->material_type->name}}</h3>
                    <p class=""></p>
                    <textarea name="" id="" cols="30" rows="5" class="text-left resize-none text-sm text-gray-500 hover:text-gray-600 leading-6">Lorem ipsum dolor sit amet consectetur adipisicing elit. Reprehenderit, eligendi dolorum sequi illum qui unde aspernatur non deserunt</textarea>
                    ...
                    <ul
                        ...
                        <li class="flex items-center py-3">
                            ...
                            @if ($material->existency > 0)
                                <span class="ml-auto"><span class="bg-green-500 py-1 px-2 rounded text-white text-sm">En Bodega</span></span>
                            @else
                                <span class="ml-auto"><span class="bg-red-500 py-1 px-2 rounded text-white text-sm">A pedido</span></span>
                            @endif
                            
                        </li>
                        <li class="flex items-center py-3">
                            <span>Existencia</span>
                            <span class="ml-auto">{{$material->existency}} {{$material->material_type->dimensional->name}}</span>
                        </li>
                    </ul>
                </div>
                <!-- End of profile card -->
                    ...
                    <div class="grid grid-cols-2">
                        @foreach ($similar as $mat)
                            ...
                            <div class="text-center my-2">
                                @if (isset($material->image->url))
                                    <img class="h-16 w-16 rounded-full mx-auto" src="{{Storage::url($mat->image->url)}}">
                                @else
                                    <img class="h-16 w-16 rounded-full mx-auto" src="{{Storage::url($mat->material_type->image->url)}}">
                                @endif
                    ...
                    <div class="text-gray-700">
                        <div class="grid md:grid-cols-2 text-sm">
                            ...
                            <div class="grid grid-cols-2">
                                ...
                                <div class="px-4 py-2">{{$material->material_type->dimensional->name}} ({{$material->material_type->dimensional->abrev}})</div>
                            </div>
                            <div class="grid grid-cols-2">
                                <div class="px-4 py-2 font-semibold">Maximo en Bodega</div>
                                <div class="px-4 py-2">{{$material->max}} ({{$material->material_type->dimensional->abrev}})</div>
                            </div>
                            <div class="grid grid-cols-2">
                                <div class="px-4 py-2 font-semibold">Minimo en Bodega</div>
                                <div class="px-4 py-2">{{$material->min}} ({{$material->material_type->dimensional->abrev}})</div>
                            </div>
                            <div class="grid grid-cols-2">
                            ...
                            <div class="grid grid-cols-2">
                                ...
                                <div class="px-4 py-2">{{$material->existency}} ({{$material->material_type->dimensional->abrev}})</div>
                            </div>
                        </div>
                    </div>
                    <button
